...
def run_fl(donnees_clients, mode_type, fabric_connector):
    ...
    # Contexte TenSEAL (chiffrement homomorphe)
    ...
    for round_num in range(1, 6):  # 5 rounds
        ...
        # Opacus activé à partir du round 3
        use_dp = True
        if use_dp:
        ...
        for i in range(4):
            ...
            # Pour les rounds avec DP, on garde 4000 échantillons (pas trop peu)
            n_eff = 4000 if use_dp else 5000
            X_tr_eff = X_tr[:n_eff]
            y_tr_eff = y_tr[:n_eff]
            modeles[i] = entrainer_client_local(
                modeles[i], X_tr_eff, y_tr_eff,
                epochs=3,  # toujours 3 epochs pour permettre l'apprentissage
                use_dp=use_dp
            )
        ...
        for i in range(4):
            ...
            # TenSEAL : chiffrement des 50 premiers poids
...
print("\n\n========== EXÉCUTION IID AVEC OPACUS (ÉQUILIBRÉ) ==========")
f1_iid = run_fl(donnees_iid, "IID", fabric)

print("\n\n========== EXÉCUTION NON-IID AVEC OPACUS (ÉQUILIBRÉ) ==========")
...
plt.plot(range(1,6), f1_iid, 'b-o', label='IID avec Opacus')
plt.plot(range(1,6), f1_non_iid, 'r-s', label='Non-IID avec Opacus')
plt.xlabel('Round FL')
plt.ylabel('F1-score macro')
plt.title('Comparaison IID vs Non-IID avec Opacus (équilibré)')
plt.legend()
plt.grid(True, alpha=0.3)
plt.ylim(0, 1.05)
plt.savefig('comparaison_iid_non_iid_opacus_equilibre.png', dpi=150)
print("\n✅ Graphique sauvegardé sous 'comparaison_iid_non_iid_opacus_equilibre.png'")
