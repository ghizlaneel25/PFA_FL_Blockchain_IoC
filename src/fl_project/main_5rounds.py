...
def run_fl(donnees_clients, mode_type, fabric_connector):
    ...
    # Contexte TenSEAL (pour démonstration, mais désactivé dans ce test)
    ...
    # ⚠️ 5 rounds au lieu de 10 (pour accélérer)
    for round_num in range(1, 6):
        print(f"\n--- Round {round_num} ({mode_type}) ---")
        
        # Pas de sécurité (DP désactivée)
        use_dp = False
        
        for i in range(4):
            X_tr, y_tr, _, _ = clients_data_split[i]
            n_eff = 5000  # toujours 5000 car pas de DP
            X_tr_eff = X_tr[:n_eff]
            y_tr_eff = y_tr[:n_eff]
...
plt.plot(range(1,6), f1_iid, 'b-o', label='IID (5 rounds)')
plt.plot(range(1,6), f1_non_iid, 'r-s', label='Non-IID (5 rounds)')
plt.xlabel('Round FL')
plt.ylabel('F1-score macro')
plt.title('Comparaison IID vs Non-IID (sans sécurité)')
plt.legend()
plt.grid(True, alpha=0.3)
plt.ylim(0, 1.05)
plt.savefig('comparaison_iid_non_iid_sans_securite.png', dpi=150)
print("\n✅ Graphique sauvegardé sous 'comparaison_iid_non_iid_sans_securite.png'")
