...
def run_fl(donnees_clients, mode_type, fabric_connector):
    ...
    # ⚡ 5 rounds seulement (round 1 à 5)
    for round_num in range(1, 6):
        print(f"\n--- Round {round_num} ({mode_type}) ---")
        
        # Opacus activé seulement à partir du round 3 (pour gagner du temps)
        use_dp = (round_num >= 3)
        if use_dp:
        ...
        for i in range(4):
            ...
            # ⚡ Réduction des données pour les rounds avec DP (1000 au lieu de 2000)
            n_eff = 1000 if use_dp else 5000
            X_tr_eff = X_tr[:n_eff]
            y_tr_eff = y_tr[:n_eff]
            modeles[i] = entrainer_client_local(modeles[i], X_tr_eff, y_tr_eff, epochs=2 if use_dp else 3, use_dp=use_dp)
...
plt.plot(range(1,6), f1_iid, 'b-o', label='IID')
plt.plot(range(1,6), f1_non_iid, 'r-s', label='Non-IID')
plt.xlabel('Round FL')
