...
def run_fl(donnees_clients, mode_type, fabric_connector):
    ...
    # 5 rounds – pas de DP, uniquement TenSEAL
    for round_num in range(1, 6):
        ...
        for i in range(4):
            ...
            # On garde 5000 échantillons pour tous les rounds (pas de DP)
        ...
        for i in range(4):
            ...
            # (Le chiffré n'est pas stocké, mais on montre qu'il est calculé)
...
print("\n\n========== EXÉCUTION IID (TenSEAL uniquement) ==========")
f1_iid = run_fl(donnees_iid, "IID", fabric)

print("\n\n========== EXÉCUTION NON-IID (TenSEAL uniquement) ==========")
...
plt.plot(range(1,6), f1_iid, 'b-o', label='IID (TenSEAL)')
plt.plot(range(1,6), f1_non_iid, 'r-s', label='Non-IID (TenSEAL)')
plt.xlabel('Round FL')
plt.ylabel('F1-score macro')
plt.title('Comparaison IID vs Non-IID (chiffrement TenSEAL uniquement)')
plt.legend()
plt.grid(True, alpha=0.3)
plt.ylim(0, 1.05)
plt.savefig('comparaison_iid_non_iid_tenseal_only.png', dpi=150)
print("\n✅ Graphique sauvegardé sous 'comparaison_iid_non_iid_tenseal_only.png'")
