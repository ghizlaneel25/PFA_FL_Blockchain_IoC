...
def run_fl(donnees_clients, mode_type, fabric_connector):
    ...
    # Contexte TenSEAL pour le chiffrement homomorphe
    ...
    # 5 rounds pour IID et Non-IID
    for round_num in range(1, 6):
        print(f"\n--- Round {round_num} ({mode_type}) ---")

        # Activation d'Opacus à partir du round 3 (pour économiser du temps)
        use_dp = (round_num >= 3)
        if use_dp:
        ...
        for i in range(4):
            ...
            # Réduire les données pour les rounds avec DP (1000 échantillons)
            n_eff = 3000 if use_dp else 5000
            X_tr_eff = X_tr[:n_eff]
            y_tr_eff = y_tr[:n_eff]
            # Avec DP, on utilise 2 epochs au lieu de 3 pour accélérer
            modeles[i] = entrainer_client_local(
                modeles[i], X_tr_eff, y_tr_eff,
                epochs=2 if use_dp else 3,
                use_dp=use_dp
            )

        # Récupérer les poids de tous les clients
        tous_les_poids = [modele.state_dict() for modele in modeles]

        # Agrégation décentralisée (FedAvg local) et enregistrement sur Fabric
        for i in range(4):
            # Calcul de la moyenne des poids (FedAvg)
            poids_moyen = {}
            ...
            # Hash des poids agrégés pour la traçabilité
...
print("\n\n========== EXÉCUTION IID AVEC SÉCURITÉ ==========")
f1_iid = run_fl(donnees_iid, "IID", fabric)

print("\n\n========== EXÉCUTION NON-IID AVEC SÉCURITÉ ==========")
...
# Graphique de comparaison IID vs Non-IID
plt.figure(figsize=(10,6))
plt.plot(range(1,6), f1_iid, 'b-o', label='IID avec sécurité')
plt.plot(range(1,6), f1_non_iid, 'r-s', label='Non-IID avec sécurité')
plt.xlabel('Round FL')
plt.ylabel('F1-score macro')
plt.title('Comparaison IID vs Non-IID avec Sécurité (5 rounds)')
plt.legend()
plt.grid(True, alpha=0.3)
plt.ylim(0, 1.05)
plt.savefig('comparaison_iid_non_iid_securite_5rounds.png', dpi=150)
print("\n✅ Graphique sauvegardé sous 'comparaison_iid_non_iid_securite_5rounds.png'")
